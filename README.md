...
- 애플리케이션 레벨 미들웨어
  - app.use 또는 app.METHOD를 이용하여 등록하는 미들웨어
- 라우터 레벨 미들웨어
  - express.Router로 생성된 라우트에 등록하는 미들웨어
- 오류 처리 미들웨어
  - 오류를 처리하기 위해 등록하는 미들웨어
  - 반드시 인수 4개를 제공 하여야 하며, 인수가 4개가 아닌 경우 일반 미들웨어로 인식한다.
- 기본 제공 미들웨어
  - express에서 기본으로 제공하는 미들웨어
  - 4.x부턴 express.static 유일하며, 정적 자산을 제공하는 역할을 한다.(그 외에는 별도 모듈로 분리)
- 써드파티 미들웨어
